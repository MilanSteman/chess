...
class Pawn extends Piece {
   ...
   */
  private forwardMoves(moves: PotentialMove[]): void {
    // Check how many tiles the pawn can move (1 or 2 depending on whether it has moved)
    const distance: number = !this.hasMoved ? 2 : 1;

    for (let i = 1; i <= distance; i++) {
      // Calculate new move position
      const newRow: number = this.row + i * this.direction;

      // Check if the new position is within the bounds of the chessboard
      if (isPositionInBounds(newRow, this.col)) {
        const targetPiece: Piece | null = Board.grid[newRow][this.col];
  ...
  private captureMoves(moves: PotentialMove[]): void {
    for (const [x, y] of [[this.direction, -1], [this.direction, 1]]) {
      ...
      if (isPositionInBounds(newRow, newCol)) {
        const targetPiece: Piece | null = Board.grid[newRow][newCol];
   ...
   */
  private enPassantMoves(moves: PotentialMove[]): void {
   ...
   */
  private canEnPassant(lastMove: MadeMove): boolean {
