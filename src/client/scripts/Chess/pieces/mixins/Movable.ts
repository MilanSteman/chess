...
interface Moveable {
  ...
  isMoveValid: (instance: Piece, move: PotentialMove | undefined, newRow: number, newCol: number) => boolean;
  ...
  setMadeMove: (instance: Piece, move: PotentialMove, isCapture: Piece | null, castledPiece: Piece | null) => MadeMove;
...
const MoveableMixin: Moveable = {
  ...
  makeMove(instance: Piece, newRow: number, newCol: number): void {
    ...
    const legalMove: PotentialMove | undefined = instance.getLegalMoves().find(
    ...
    if (legalMove) {
      // Get castled piece
      const castledPiece: Piece | null = legalMove?.specialMove === CastlingType.LONG || legalMove?.specialMove === CastlingType.SHORT
        ? this.handleCastling(instance, legalMove)
        : null;

      // Check if it is an capture and handle accordingly
      const isCapture: Piece | null = legalMove?.specialMove === "en-passant"
        ? this.handleEnPassant(instance, legalMove)
        : this.handleTarget(newRow, newCol);

      let madeMove: MadeMove = this.setMadeMove(instance, legalMove, isCapture, castledPiece);
      ...
      madeMove = { ...madeMove, ...{ checkmate: Game.state.endType === GameEndTypes.CHECKMATE } };
      ...
      client.socket.emit("move", { fromRow, fromCol, toRow, toCol }, Game.currentPlayer?.color);
    }
  },
  ...
  handleCastling(instance: Piece, move: PotentialMove): Piece | null {
    ...
    translate(castlingRook?.pieceDomEl, DEFAULT_TRANSLATE, colDiff * TRANSLATE_VAL);
    ...
    setTimeout(() => {
      ...
      castlingRook?.MoveableMixin.updatePiecePosition(castlingRook, castlingRook.row, castlingRook.col + colDiff);
    }, Game.moveSpeed);
   ...
   */
  handleEnPassant(instance: Piece, move: PotentialMove): Piece | null {
    const direction: number = instance.color === Players.WHITE ? PawnDirection.BLACK : PawnDirection.WHITE;
  ...
  updatePiecePosition(instance: Piece, newRow: number, newCol: number): void {
    ...
    instance.parentDomEl = document.querySelector(`div.tile[data-tile="${getTileFromPosition(newRow, newCol)}"]`) as HTMLElement;
  ...
  createMoveElement(move: MadeMove): void {
    ...
    const moveListEl: HTMLDivElement | null = document.querySelector(".side > .list");
